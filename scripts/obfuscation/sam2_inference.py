...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        # default="/home/m/CIIRC/sam2/checkpoints/sam2.1_hiera_large.pt",
        default="/home/m/CIIRC/sam2/checkpoints/sam2.1_hiera_base_plus.pt",
        help="Path to the SAM checkpoint file",
    )
    parser.add_argument(
        "--config",
        type=str,
        choices=["configs/sam2.1/sam2.1_hiera_l.yaml", "configs/sam2.1/sam2.1_hiera_b+.yaml", "configs/sam2.1/sam2.1_hiera_s.yaml", "configs/sam2.1/sam2.1_hiera_t.yaml"],
        default="configs/sam2.1/sam2.1_hiera_b+.yaml",
        help="Path to the SAM config file",
    )
    parser.add_argument(
        "--points_per_side",
        type=int,
    ...
    mask_generator = SAM2FilteredMaskGenerator(
        model=sam2,
        ...
        use_m2m=True,  # one step refinement using previous mask predictions
    )
    ...
    for input_path, output_path in tqdm(zip(input_paths, output_paths), total=len(input_paths)):
        ...
        masks = mask_generator.generate(img, filter_mask)
...
class SAM2FilteredMaskGenerator(SAM2AutomaticMaskGenerator):
    @torch.no_grad()
